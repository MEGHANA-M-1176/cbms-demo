...
content = content.replace(
  /Briefcase, AlertCircle, Clock, Activity, Calendar\n\} from 'lucide-react';/,
...
content = content.replace(/export const Loans: React\.FC = \(\) => \{/, newComponents + '\nexport const Loans: React.FC = () => {');
...
// 4. Update fetchLoans
content = content.replace(
  /const assignedRes = await apiClient\.get\('\/loans\/assigned'\);/,
...
content = content.replace(/const handlePayEmi = async \(e: React\.FormEvent\) => \{/, handlers + '\n  const handlePayEmi = async (e: React.FormEvent) => {');
...
content = content.replace(/<div style=\{\{ display: 'flex', flexDirection: 'column', gap: '24px' \}\}>/, tabsHtml + '        <div style={{ display: \'flex\', flexDirection: \'column\', gap: \'24px\' }}>');

// 7. Add PENDING and APPROVED content blocks right after the MY_PORTFOLIO block ends
...
content = content.replace(/\{\/\* New Loan Modal \*\/\}/, tabsContentHtml + '\n      {/* New Loan Modal */}');
